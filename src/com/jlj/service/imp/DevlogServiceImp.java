...
//		queryString += " order by mo.id desc ";
	...
	public int getSigTotalCount(String number) {
		String queryString = "select count(*) from Devlog mo where mo.sig.number=? ";
		Object[] p = new Object[]{number};
		return devlogDao.getUniqueResult(queryString, p);
	}
	public List<Devlog> querySigList(String number, int page, int size) {
		String queryString = "from Devlog mo where mo.sig.number=? ";
		Object[] p = new Object[]{number};
		return devlogDao.pageList(queryString, p, page, size);
	}
